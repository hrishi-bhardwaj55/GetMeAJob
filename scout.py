    ...
    def fetch_jobs(self, keywords, location="United States"):
        ...
        with sync_playwright() as p:
            ...
            for keyword in keywords:
                # To bypass LinkedIn's 60-job limit per query, we paginate by time.
                # The user requested 0-10, 11-20, 21-30, 31-40, 41-50, 51-60 minutes ago.
                # r600 = Past 10 min, r1200 = Past 20 min, etc.
                time_offsets_seconds = [600, 1200, 1800, 2400, 3000, 3600]
                
                for offset in time_offsets_seconds:
                    params = {
                        "keywords": keyword,
                        "location": location,
                        "f_TPR": f"r{offset}", 
                    ...
                    print(f"[Scout Agent] Searching for: '{keyword}' (Past {offset // 60} mins)...")
                    ...
                    try:
                        ...
                        # Keep scrolling until no new jobs are loaded, exhausting the specific time feed
                        ...
                        print(f"[Scout Agent] Found {len(job_cards)} job cards for '{keyword}' in past {offset // 60} mins. Extracting details...")
                        ...
                        print(f"[Scout Agent] Error fetching jobs for {keyword} (Past {offset // 60} mins): {e}")
                        
                    self._human_delay(3, 6) # Small delay between time offsets within the same keyword
